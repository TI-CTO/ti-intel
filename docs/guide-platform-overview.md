...
```
  ...
  └──────────────────┬───────────────────────────────┘
                     ...
                     ▼
  ┌──────────────────────────────────────────────────┐
  │        WTIS v4.1 Full (종합 기술 전략 제안서)          │
  │  전략 옵션 + 후보 기업 + ROI를 하나의 문서로 통합      │
  │  "어떻게, 누구랑, 어떻게 도입할 것인가?"              │
  └──────────────────┬───────────────────────────────┘
                     │
                     ▼
...
## 3. WTIS v4.1 — 과제 검증 시스템 (2-Tier)

### 목적

특정 L2 기술에 대해 **Go/No-Go 판정**, **200점 채점**, **전략+기업+재무 종합 분석**을 산출한다.
"하나의 판정 = 하나의 의사결정" 원칙.
...
| Tier | 모드 | 용도 | 소요 | 산출물 |
|------|------|------|------|--------|
| **Standard** | proposal / standard / deep | 기술 스크리닝 Go/No-Go | ~15분 | 기술 평가 리포트 |
| **Full** | full | CTO 종합 기술 전략 제안서 | ~30분 | 기술+전략+기업+재무 통합 리포트 |

- **Standard**: 모든 기술에 수시로 실행. Go/No-Go만 빠르게 판정
- **Full**: Go/Conditional Go 기술만 대상. 전략 옵션(Build/Buy/Partner) + 후보 기업 + ROI까지 **하나의 문서**로 통합

> 상세: [WTIS 2-Tier 가이드](docs/guide-strategy-analysis.md)
...
Standard 리포트 읽기
  → research-deep × 3~4 (Build/Buy/Partner + 시장·비용)
  → 전략 옵션 스코어링 (5기준 × 20점)
  → 후보 기업 분석 (deal_stage 연동)
  → ROI 3시나리오 모델링
  → fact-checker 팩트 체크 (핵심 주장 반례 탐색)
  → validator 교차검증 (내부 일관성)
  → 종합 기술 전략 제안서 (1파일) + PDF
```
...
제안서/기술명 입력
  ...
  ※ Full 모드에서는 fact-checker(외부 사실 검증)가 validator 전에 추가 실행
...
|------|------|------|
| 160~200 | **Go** | 추진 권고 |
| 120~159 | **Conditional Go** | 조건부 추진 (리스크 해소 필요) |
| 80~119 | **재검토** | 근본적 재설계 필요 |
| 1~79 | **No-Go** | 보류 |
