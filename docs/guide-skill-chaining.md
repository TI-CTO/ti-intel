...
└──────────────┘                │ (~30분)          │
  ...
  ※ strategy-options, biz-case는 독립 실행도 가능 (Full이 내부 통합)
...
### 시나리오 4-b: 개별 스킬로 분리 실행 (필요 시)
```
/strategy-options {wtis 리포트}        ← 전략 옵션만 깊이 검토
/biz-case {strategy-options 리포트}    ← 재무 모델만 별도 갱신
```
