...
MCP 병렬 탐색: intel-store(논문 20건, 특허 18건, 뉴스 20건)
...
**MCP Sources:**
- `intel-store`: `collect_papers(topic=..., query=..., limit=20)` — 학술 논문 수집+저장
- `intel-store`: `collect_patents(topic=..., query=..., limit=20)` — 특허 수집+저장
- `intel-store`: `collect_news(topic=..., query=..., limit=20)` — 뉴스 수집+저장
- `intel-store`: `search_intel(query=..., mode="semantic", limit=20)` — 통합 시맨틱 검색
...
## 기술 동향 신호
### 학술 동향 (intel-store: papers)
### 특허 동향 (intel-store: patents)
### 시장/뉴스 동향 (intel-store: news + WebSearch)
