...
```
- Layer 2 열: 첫 번째 L3에만 Layer 2 이름 표기, 같은 그룹의 후속 L3는 빈칸.
- 모든 L3를 표시 (🟢 포함). 분석(Deep/Quick) 열 없음.
- 테이블 위에 `> **이번 주 핵심**: ...` 1~2줄 요약 선행.
- **정렬**: Layer 2 그룹 간 = 그룹 내 최고 신호 기준 (🔴 > 🟡 > 🟢). 동일 신호면 알파벳 순. Layer 2 그룹 내 = 신호 높은 순. 긴급 기술이 테이블 상단에 온다.
- **카테고리 태그**: 핵심 내용 앞에 `[태그]` 접두사로 시그널 성격을 표시. 하나의 셀에 복수 태그 가능.
  - `[기술돌파]` — 새로운 기술 성과, 벤치마크 갱신
  - `[제품출시]` — 상용 제품/서비스 런칭
  - `[경쟁사]` — SKT/KT 등 경쟁사 움직임
  - `[규제]` — 법·규제·표준 변경
  - `[투자]` — 펀딩, M&A, 대규모 투자
  - `[논문]` — 학술적 발견, 주요 논문
  - `[생태계]` — 파트너십, 오픈소스, 플랫폼 변화
  - 🟢 평온 항목은 태그 생략 가능 ("변화 없음" 등 단순 기술).
...
### T4: 시장 시그널
- **테이블 금지** — 불릿 리스트로만 작성.
- **시그널 유형별 볼드 소제목**으로 구조화:
  - **투자 & M&A** — 펀딩 라운드, 인수, 밸류에이션
  - **파트너십 & 제휴** — 기업 간 협업, 통합, OEM
...
> **이번 주 핵심**: {도메인 전체를 관통하는 1~2줄 핵심 메시지}

| Layer 2 | 세부기술 | 신호 | 핵심 내용 |
|---------|----------|------|----------|
| {L2 이름 — 🔴 포함 그룹 먼저} | {L3 이름} | 🔴 | [제품출시] {핵심 내용} · [규제] {핵심 내용} |
| | {L3 이름} | 🟡 | [기술돌파] {핵심 내용} |
| {L2 이름 — 🟡 그룹} | {L3 이름} | 🟡 | [논문] {핵심 내용} |
| | {L3 이름} | 🟢 | {핵심 내용 — 태그 생략 가능} |
| {L2 이름 — 🟢 그룹} | {L3 이름} | 🟢 | 변화 없음 |
...
**투자 & M&A**
- {펀딩/인수 시그널} [[G-xx]](#ref-g-xx)

**파트너십 & 제휴**
- {협업/통합 시그널} [[G-xx]](#ref-g-xx)

**시장 전망**
- {TAM/CAGR/예측 시그널} [[G-xx]](#ref-g-xx)

**도입 사례**
- {프로덕션 배포/PoC 시그널} [[G-xx]](#ref-g-xx)

**연구 동향**
- {논문 제목} ({저자, 연도}) — {시사점 1줄} [[P-xx]](#ref-p-xx)
...
## Writing Style (가독성 규칙)

### 핵심 메시지 압축
- **Executive Summary의 "이번 주 핵심"은 3문장 이내로 구조화한다.** 경쟁축/변화 방향/시사점을 명확히 분리.
- Deep 각 L3의 첫 문단은 "이번 주 이 기술에서 무엇이 바뀌었는가"를 한 문장으로 선언한다.
- 기술 동향의 각 항목은 **볼드 제목 — 한 줄 요약** 패턴을 엄격히 지킨다. 상세 설명은 그 아래에 분리.

### 문장 리듬
- 한 문장에 수치 2개 이상 나열 금지. 수치가 많으면 테이블이나 불릿으로 분리한다.
- 괄호 중첩 금지: `(A (B))` 패턴 → 별도 문장으로 풀어 쓴다.
- 기술 용어 나열 시 3개 이상이면 불릿 리스트로 전환한다.
  - 나쁜 예: "Dual-AR + RL 정렬, 80언어, Slow AR(4B, 시간축 의미 코드북)과 Fast AR(400M, 잔여 코드북)의 비대칭 아키텍처"
  - 좋은 예: 불릿 3개로 분리

### 약어 풀어쓰기 강화
- **Step 4.6 실행을 건너뛰지 않는다.** PDF 변환 전 반드시 약어 검증을 수행한다.
- Executive Summary에서 사용한 약어라도 본문 첫 등장 시 반드시 다시 풀어 쓴다.
- 약어가 10개 이상이면 리포트 상단에 약어표(Glossary)를 추가한다.

### 독자 기준
- 이 리포트의 1차 독자는 **CTO/경영진**이다. 기술 상세는 Deep에 넣되, Executive Summary와 시사점은 비전공자도 이해 가능한 수준으로 쓴다.
- "~하고 있다" 나열보다 "이것이 우리에게 의미하는 바는 X다"로 마무리한다.
