...
outputs/reports/weekly/
  YYYY-MM-DD_weekly-{domain-slug}.md              ← 메인 리포트 (Tier 1 + Tier 2 요약)
  YYYY-MM-DD_weekly-{domain-slug}.professional.pdf ← PDF
  YYYY-MM-DD_research-{l3-slug}.md                ← Tier 2 심층 리서치 (L3별 개별 파일)
```

⚠️ **Tier 2 research-deep 에이전트 호출 시 `file_path`를 반드시 `weekly/` 경로로 지정한다.**
