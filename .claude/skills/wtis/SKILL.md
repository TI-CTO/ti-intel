...
- **선행 리서치 합칠 때**: 충돌하는 ID는 순차 재번호 매기기로 해소. 접미사 붙이지 않는다.
- **유형별 01부터 시작**: G(글로벌/뉴스), P(논문), E(기업 발언), N(동향), T(특허), I(내부).
- **이유**: 접미사 ID는 마크다운 파서 호환성 문제, PDF 뱃지 렌더링 오류, 검증 체계 불일치를 유발한다.

### PDF 렌더링 호환 규칙

- **코드 블록 안에 체크리스트/액션 아이템 넣지 않는다**: ` ``` ` 안의 `- [ ]`는 raw 텍스트로 렌더링된다. 일반 마크다운 리스트로 작성할 것.
- **테이블 셀 텍스트는 간결하게**: 5열 이상 테이블에서 한 셀에 장문 텍스트를 넣으면 레이아웃 붕괴. 핵심 키워드만 넣고 상세 설명은 본문에 서술.
...
## Executive Summary
> 카드 UI(판정/점수/전략)는 frontmatter에서 자동 생성. 본문은 아래 5항목을 **볼드 라벨 + 1줄 요약** 형식으로 작성:
>
> **시장**: TAM/CAGR 핵심 수치 + 출처
> **기술**: TRL 수준 + 주요 벤치마크 수치
> **기회**: 차별화 포인트 (규제, 망 통합 등)
> **위협**: 경쟁 선점 현황 + 핵심 리스크
> **권고**: 3B 전략 1줄 요약 + 벤치마크 모델
