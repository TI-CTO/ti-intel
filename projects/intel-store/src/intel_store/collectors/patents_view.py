...
def search_patents(
    ...
    *,
    ...
    try:
        ...
        logger.error("PatentsView API error: %s %s", e.response.status_code, e.response.text[:200])
        return []
    except httpx.RequestError as e:
        logger.error("PatentsView request failed: %s", e)
        return []
...
def _normalise(raw: dict) -> dict | None:
    """Convert a raw PatentsView patent dict to our schema."""
